...
/**
 * A scope function that would be executed for prettify error response if the request failed.
 *
 * @param onError The receiver function that receiving prettified error message
 *
 * @return The original [ApiResponse].
 */
@JvmSynthetic
inline fun <T> ApiResponse<T>.handleError(
...
/**
 * A scope function that would be executed for handling error responses if the request failed with a [StatusCode].
 *
 * @param specificStatusCode The [StatusCode] that run result when error code is equal with
 * @param onResult The receiver function that receiving [ApiResponse.Failure.Exception] if the request failed.
 *
 * @return The original [ApiResponse].
 */
@JvmSynthetic
inline fun <T> ApiResponse<T>.onSpecificStatus(
    specificStatusCode: StatusCode,
    crossinline onResult: ApiResponse.Failure.Error<T>.() -> Unit
): ApiResponse<T>? {
    if (this is ApiResponse.Failure.Error) {
        if (statusCode == specificStatusCode) {
            onResult(this)
            return null
        }
...
fun <T> ApiResponse.Failure.Error<T>.message(): String {
    ...
    response.errorBody()?.string()?.let {
        val errorMap = Cartographer.INSTANCE.fromJson(it)
        errorMap.keys.forEach { key ->
            val entry = errorMap[key]
            errorMessageBuilder.append(mapErrorMessage(entry))
        }
    }
...
private fun mapErrorMessage(entry: Any?): String {
    val errorMessageBuilder = StringBuilder("")
    when (entry) {
        is String -> {
            errorMessageBuilder.append("\n")
            errorMessageBuilder.append(entry)
        }
        is List<*> -> {
            entry.forEach { entryListItem ->
                errorMessageBuilder.append(mapErrorMessage(entryListItem))
            }
        }
        is Map<*, *> -> {
            entry.keys.forEach { key ->
                errorMessageBuilder.append(mapErrorMessage(entry[key]))
            }
        }
    }
